...
export function StatsPanel() {
  ...
  if(!open)return null;
  return <section className="f-page f-colophon"><header className="f-colophon-header"><span className="f-kicker">04 / A RECORD OF THE WORK</span><h1>The colophon.</h1><p>Small things, faithfully done.</p></header><StatsInner/></section>;
}
...
function StatsInner() {
  ...
  return (
    <div className="f-colophon-grid"><div className="f-colophon-tools">
      <div>
        <Eyebrow right={<button className="w-icon-btn" onClick={() => setUI({ statsOpen: false })} aria-label="Close"><Icon name="close" size={13} /></button>}>The record</Eyebrow>
        {/* the level stack — the panel's statement moment */}
        <div className="w-statement f-level-statement" style={{ ...statementBg("level-" + derived.level.lvl), padding: "14px 16px 16px" }}>
          ...
          <div style={{ margin: "12px 0 6px" }}>
            <div style={{ height: 6, background: "rgba(20,16,10,0.25)" }}>
              <div style={{ height: "100%", width: derived.level.pct + "%", background: "var(--amber)", backgroundImage: "none" }} />
            </div>
          </div>
    ...
    </div><aside className="f-trophies">      <div>
        <Eyebrow>Impressions earned · {achievements.length}/{D.ACHIEVEMENTS.length}</Eyebrow>
        <div className="f-achievement-cabinet">
          {D.ACHIEVEMENTS.map((a,index) => {
            ...
            return (
              <div key={a.id} className={'f-achievement'+(done?' is-earned':'')}><span className="f-kicker">NO. {String(index+1).padStart(2,'0')} / {done?'COLLECTED':'IN THE MAKING'}</span><BookArt index={index} progress={done?100:pct}/>
