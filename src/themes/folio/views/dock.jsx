...
export function DockView() {
  ...
  return (
    <div className="f-page f-collection">
      <PageHeader
        eyebrow="03 / THE COLLECTION"
        title={tab === "projects" ? "Works in progress." : "The manuscript index."}
        sub={tab === "projects" ? "Good things take shape, one page at a time." : "Every thought has a place. Find yours."}
        right={
...
function LaunchNoteInner({ projectId }) {
  ...
  useEffect(() => {
    ...
    aiLaunchNote(project, stats || {}).then((text) => {
      if (alive && text && !noteTouched.current) setNote(text);
  ...
  return (
    <div className="w-backdrop" onClick={() => setUI({ launchNoteFor: null })}
      ...
      <div onClick={(e) => e.stopPropagation()} className="w-console w-fade-in f-publication" role="dialog" aria-modal="true" aria-label="Publication note" style={{ width: "100%", maxWidth: 470 }}>
        <div className="w-console-head">
          <span className="w-tape w-tape--starboard"><Icon name="ship" size={10} /> Publication note</span>
          <div style={{ flex: 1 }} />
        ...
        <div style={{ padding: "16px 18px 18px" }}>
          ...
          <div className="w-well" style={{ marginBottom: 6 }}>
            <textarea value={note} onChange={(e) => { noteTouched.current=true; setNote(e.target.value); }}
...
function ProjectsTab() {
  useEffect(() => { registerActiveList("projects", []); }, []);
      ...
      {active.length === 0 && !adding && (
        <EmptyState>
          {activeAnyType && typeFilter !== "All"
            ? <>No {D.PROJECT_TYPES.find((t) => t.id === typeFilter)?.label.toLowerCase()} projects in the collection.</>
            : <>Your collection starts here. Create one, or capture with the Project mode.</>}
        </EmptyState>
      )}

      <div className="f-bookshelf">
        {active.map((p, volumeIndex) => {
          ...
          return (
            <article key={p.id} className={"f-volume"+(isOpen?" is-open":"")} style={{"--volume-color":p.color,"--book-color":`color-mix(in srgb, ${p.color} 48%, #33242d)`}}>
              <button className="f-volume-cover" onClick={()=>setExpandedId(isOpen?null:p.id)} aria-expanded={isOpen}>
                <span className="f-volume-spine">{String(volumeIndex+1).padStart(2,'0')}</span>
                <span className="f-volume-title"><small>{typeLbl} / VOLUME {String(volumeIndex+1).padStart(2,'0')}</small><strong>{p.title}</strong><span>{isReady?'Ready to publish':children.length+' tasks in this volume'}</span></span>
                <BookArt index={[...p.id].reduce((n,c)=>n+c.charCodeAt(0),0)%31} progress={pct}/><span className="f-volume-progress"><span><b>{pct}</b>%</span><span className="f-volume-lines">{Array.from({length:20},(_,i)=><i key={i} className={i<pct/5?'printed':''}/>)}</span><small>{doneCount} / {children.length} complete</small></span>
                <span className="f-volume-open">{isOpen?'−':'+'}</span>
              </button>
              {isOpen && (
                <div className="f-volume-contents"><div className="f-kicker">THE CONTENTS / A WORK IN PROGRESS</div>
        ...
        <div style={{ marginTop: 30 }}>
          <div className="w-stencil-row" style={{ marginBottom: 12 }}>
            <button className="w-stencil" onClick={() => setShowLaunched(!showLaunched)}
              ...
              Published editions · {launched.length}
            </button>
          </div>
          {showLaunched && (
                    ...
                    {open && (
                      <div className="w-fade-in" style={{ margin: "8px -10px -8px -14px", padding: "10px 14px 12px 24px", background: "var(--well)", boxShadow: "inset 0 1px 0 var(--line-dim)" }}>
                        ...
                        <div style={{ display: "flex", gap: 6 }}>
                          ...
                          <div style={{ flex: 1 }} />
                          <button className="w-bezel w-bezel--sm" onClick={() => unshipProject(p.id)} title="Return to works in progress">Unpublish</button>
                        </div>
                      </div>
                    )}
                  </div>
            ...
            {parsed.map((p, i) => {
              ...
              return (
                <div key={i} onClick={() => setImportSel((prev) => { const n = new Set(prev); n.has(i) ? n.delete(i) : n.add(i); return n; })}
                  ...
                  <Checkbox checked={on} label={"Include "+p.title} onChange={() => setImportSel(prev => {const next=new Set(prev);next.has(i)?next.delete(i):next.add(i);return next;})} />
