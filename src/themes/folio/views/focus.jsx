...
function Tunnel({ taskId }) {
  ...
  return <div className="w-focus-overlay" role="dialog" aria-modal="true" aria-label="Focus mode">
    ...
    <div className="f-focus-stage"><div className="f-kicker">{project?project.title+' / ':''}{task.difficulty} · ~{D.taskHours(task)}h estimated</div><h1 className="w-focus-title">{task.title}</h1><FocusDial elapsed={elapsed} hours={D.taskHours(task)} time={timeStr} running={running}/><div className="f-focus-controls"><button className="w-bezel" onClick={()=>setRunning(r=>!r)}><span className="f-play-pause" aria-hidden="true"><span className={'f-swap-icon'+(running?' is-visible':'')}>Ⅱ</span><span className={'f-swap-icon'+(!running?' is-visible':'')}><Icon name="focus" size={16}/></span></span>{running?'Pause':'Resume'} <Kbd>Space</Kbd></button><button className="w-bezel" disabled={elapsed===0} onClick={()=>{baseRef.current=0;startRef.current=Date.now();setElapsed(0);saveFocusTime(taskId,0);}}>Reset</button><button className="w-switch" onClick={complete}>Make your impression <Icon name="check"/></button></div></div>
