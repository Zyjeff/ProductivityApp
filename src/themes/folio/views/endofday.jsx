...
function Inner() {
  ...
  return (
    <div onClick={() => setUI({ endOfDayOpen: false })}
      ...
      <div onClick={(e) => e.stopPropagation()} className="w-console w-fade-in f-final-proof" role="dialog" aria-modal="true" aria-label="Close the day" style={{ width: "100%", maxWidth: 440 }}>
        ...
        <button className="w-switch" style={{ width: "100%" }}
          onClick={() => {if(mobile.trim())patchScreenToday({mobileHours:Math.max(0,Math.min(24,parseFloat(mobile)||0))});closeDay({ rollForward: rollForward && unfinished.length > 0 });}}>
