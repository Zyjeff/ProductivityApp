...
import { effectiveTodayIso, parseIsoDate } from "../../core/domain.js";
...
export function TaskFormModal() {
  ...
  return (
    <div onClick={() => setUI({ formOpen: false, editingTaskId: null })}
      ...
      <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 1120 }}>
...
function TaskForm({ initial, isEdit, onClose }) {
  ...
  return (
    <div className="w-console w-fade-in f-manuscript" role="dialog" aria-modal="true" aria-label={isEdit?"Edit task":"New task"} onKeyDown={onFormKeys}>
      ...
      <div className="f-form-spread"><section className="f-writing"><span className="f-kicker">THE MANUSCRIPT</span>
        <textarea rows={2} ref={titleRef} className="w-field f-manuscript-title" placeholder="What needs doing?"
        ...
        </section><section className="f-typesetting">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            {recurring === "none" && !isSplit && (<div>
              <div className="w-stencil" style={{ marginBottom: 6 }}>Scheduled {scheduleDate ? "" : "(planner decides)"}</div>
              <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
                <input type="date" aria-label="Scheduled date" value={scheduleDate} onChange={(e) => setScheduleDate(e.target.value)}
                  className="w-field w-field--sm" style={{ fontFamily: "var(--t-mono)", padding: "5px 7px", width: 140 }} />
                <button type="button" className="w-bezel w-bezel--sm" onClick={() => setScheduleDate(effectiveTodayIso(getState().meta.dayStartHour))}>Today</button>
                <button type="button" className="w-bezel w-bezel--sm" onClick={() => setScheduleDate(isoDate(addDays(parseIsoDate(effectiveTodayIso(getState().meta.dayStartHour)), 1)))}>Tomorrow</button>
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                <input type="date" aria-label="Deadline date" value={deadline} onChange={(e) => setDeadline(e.target.value)}
                  className="w-field w-field--sm" style={{ fontFamily: "var(--t-mono)", padding: "5px 7px", width: 140 }} />
        ...
        </section><div className="f-form-submit" style={{ display: "flex", gap: 8, marginTop: 4, alignItems: "center" }}>
...
export function ChipChoice({active,color,onClick,children}) {
  return <button type="button" className={'f-choice'+(active?' is-active':'')} aria-pressed={!!active} onClick={onClick}>{children}</button>;
}
