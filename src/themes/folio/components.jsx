...
export function Icon({ name, size = 14 }) {
  ...
  switch (name) {
    ...
    case "anchor": return <svg {...common}><path d="M8 3C6 2 4 2 2 3v10c2-1 4-1 6 0 2-1 4-1 6 0V3c-2-1-4-1-6 0zM8 3v10"/></svg>;
    case "ship": return <svg {...common}><path d="M3 2h7l3 3v9H3zM10 2v4h3M5 9h6M5 11h4"/></svg>;
...
export function CompleteButton({done,onComplete,onReopen}) {
  return <button type="button" className={'w-light'+(done?' w-light--on':'')} onClick={e=>{e.stopPropagation();(done?onReopen:onComplete)?.();}} title={done?'Reopen':'Complete'} aria-label={done?'Reopen task':'Complete task'} aria-pressed={!!done}><span className={'f-swap-icon'+(!done?' is-visible':'')} aria-hidden="true"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M12 3v18M3 12h18M5.6 5.6l12.8 12.8M5.6 18.4L18.4 5.6"/></svg></span><span className={'f-swap-icon'+(done?' is-visible':'')} aria-hidden="true"><Icon name="check" size={21}/></span></button>;
}
...
export function GhostHorizon(){return <div className="f-printers-flower" aria-hidden="true">✳</div>;}
...
export function Confetti({ onDone }) {
  ...
  const colors = ["#7c293b", "#ce593d", "#c8b596", "#344940", "#ebd6ba"];
