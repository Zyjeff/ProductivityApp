// texture.js — Kuromi's seeded dither engine, copied from the shipped
// themes (rule: themes never import each other) and retuned for the
// arcade: recipes favor ordered bayer (it reads as CRT dither), pink
// statement plates, and scanline fades. Rendered once per parameter
// set, cached as a data URL, never touched per frame.
...
export function cssColor(c, fallback = "#ff4d9e") {
...
export function tex(opts) {
  ...
  const { seed = "kuromi", algo = "bayer", w = 96, h = 96, cell = 2, fg = "#ff4d9e", bg = null, alpha = 1 } = opts;
...
// Plate under-fade: a shadow-dither pooling at a plate's foot.
export function plateFade(seedString, fg = "#241b33", from = 0.35, depth = 20) {
  return edgeTex({ seed: "fade-" + seedString, side: "bottom", depth, fg, from });
}

// Statement plate: dark dither creeping from the top and bottom edges
// of a pink surface; the center stays clean for text (legibility law).
export function statementBg(seedString, ink = "#2a0f22", pink = "#ff4d9e") {
...
// Ghost glyph fill: faint bayer field for background motifs.
export function ghostFill(seedString, w = 260, fg = "#33254a") {
  return tex({ seed: "ghost-" + seedString, algo: "bayer", w, h: 168, cell: 3, fg, grad: { from: 0.55, to: 0.08, dir: "down" } });
}

// Checker band: the 2px harlequin check used on the HUD and dividers.
export function checkerTex(seedString, fg = "#ff4d9e", alpha = 0.5) {
  return tex({ seed: "check-" + seedString, algo: "bayer", w: 16, h: 16, cell: 2, fg, density: 0.5, alpha });
}
