...
export function CaptureBar({ scheduleToday = false, autoRegister = true }) {
  ...
  return (
    <div className="f-capture" ref={wrapRef} style={{ position: "relative", width: "100%" }}>
      <div className="w-well" style={{ display: "flex", alignItems: "stretch", gap: 0 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 9, padding: "0 12px", flexShrink: 0 }}>
          ...
          <button className="w-icon-btn f-capture-submit" title="Capture task" aria-label="Save capture" onClick={submit} disabled={!text.trim()}><Icon name="plus" size={17}/></button><Kbd>N</Kbd>
        </div>
      </div>
