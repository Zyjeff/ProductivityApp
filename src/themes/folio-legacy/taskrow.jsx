...
export function TaskRow({
  ...
  return (
    <div
      className={"w-ledger-row f-task-row" + (done ? " w-ledger-row--done" : "") + (isCursor ? " w-ledger-row--cursor" : "") + (settling ? " w-settling" : "")}
    ...
    >
      ...
      <div className="f-task-main" style={{ display: "flex", alignItems: "center", gap: 10 }}>
        <CompleteButton done={done} tone={diff.tone}
          onComplete={() => onToggleDone(true)} onReopen={() => onToggleDone(false)} />
        <div className="f-task-copy" role={onToggleExpand ? "button" : undefined} tabIndex={onToggleExpand ? 0 : undefined} aria-expanded={onToggleExpand ? expanded : undefined} aria-label={"Expand "+task.title} onKeyDown={e=>{if(onToggleExpand && (e.key==="Enter"||e.key===" ")){e.preventDefault();e.stopPropagation();onToggleExpand();}}} style={{ flex: 1, minWidth: 0, cursor: onToggleExpand ? "pointer" : "default" }}
          ...
          {(task.tags?.length || project) && !compact && (
            <div style={{ display: "flex", gap: 4, marginTop: 4, flexWrap: "wrap" }}>
              {project && (
                <span className="w-stamp w-stamp--lamp" style={{ color: `color-mix(in srgb, ${project.color} 40%, #302d29)` }}>
        ...
        <div className="f-task-actions" style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
