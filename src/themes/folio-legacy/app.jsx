...
export default function App(){
  ...
  useEffect(()=>{const before=document.title;document.title='Folio Legacy — A private press for your day';return()=>{document.title=before;};},[]);
  ...
  return <div className="f-shell">
    ...
    <header className="f-masthead">
      ...
      <span className="f-brand-description">A PRIVATE PRESS<br/>FOR YOUR DAY</span>
      <nav aria-label="Main navigation">{nav.map(([id,label,key])=><button key={id} className={ui.view===id&&!ui.statsOpen?'is-active':''} onClick={()=>go(id)} title={`${id==='dock'?'Dock':id==='plan'?'Plan':'Today'} (${key})`}><sup>0{key}</sup>{label}</button>)}<button className={ui.statsOpen?'is-active':''} onClick={()=>setUI({statsOpen:!ui.statsOpen,cursor:null})} title="Logbook (4)"><sup>04</sup>Colophon</button></nav>
