    ...
    <>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {active.map((p, volumeIndex) => {
          ...
          return (
            <article key={p.id} className={"f-volume"+(isOpen?" is-open":"")} style={{"--volume-color":p.color}}>
              <button className="f-volume-cover" onClick={()=>setExpandedId(isOpen?null:p.id)} aria-expanded={isOpen}>
                ...
                <span className="f-volume-progress"><span><b>{pct}</b>%</span><span className="f-volume-lines">{Array.from({length:20},(_,i)=><i key={i} className={i<pct/5?'printed':''}/>)}</span><small>{doneCount} / {children.length} complete</small></span>
