  ...
  return <div className={"f-page f-today" + (reading ? " f-reading" : "")}>
    ...
    <section className="f-cover">
      <div className="f-cover-copy"><div className="f-kicker">A little intention. A lasting impression.</div>
        <h1>{allDone ? <>A day,<br/><em>well made.</em></> : pending.length===0 && lineup.length===0 ? <>A fresh<br/><em>impression.</em></> : <>{hourNow<12 ? 'Good morning,' : hourNow<18 ? 'Make room' : 'A little space'}<br/><em>{hourNow<12 ? 'good work.' : hourNow<18 ? 'for good work.' : 'for good work.'}</em></>}</h1>
        <p>Give your attention to what deserves it.<br/>One considered thing at a time.</p>
    ...
    <section className="f-composing">
      ...
      {nowRow ? <div className="f-now">
        <div className="f-now-label"><span className="f-kicker">ON THE PRESS</span><span className="f-now-number">{String(lineup.indexOf(nowRow)+1).padStart(2,'0')}</span></div>
