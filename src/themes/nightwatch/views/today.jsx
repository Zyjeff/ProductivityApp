...
export function TodayView() {
  ...
  const greeting = allDone ? "All clear. Harbor's quiet."
    : lineup.length === 0 && pending.length === 0 ? "Empty yard. Capture something."
    : hourNow < 5 ? "Late shift at the yard."
  ...
  const hud = [
    { label: "HARBOR CAM 01" },
    { label: "TIDE", value: stats.todayXP > 0 ? `FLOOD +${stats.todayXP}XP` : "SLACK" },
    { label: "HW", value: upcoming ? D.fmtIsoShort(upcoming.date).toUpperCase() : "—" },
    { right: true, items: [
      { label: "SWELL", value: missed.length ? `${missed.length} ADRIFT` : "0.0M" },
      { label: "VIS", value: stats.streakRun > 0 ? `${stats.streakRun}D CLEAR` : "FOG" },
    ] },
  ];
                ...
                <div className="statement ondeck" style={{ ...statementBg("ondeck-" + nowRow.task.id) }}>
                  <div className="ondeck-top">
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div className="w-stencil" style={{ marginBottom: 6 }}>Now · on deck</div>
