...
function TaskForm({ initial, isEdit, onClose }) {
  ...
  return (
    <div className="console v-inscription" onKeyDown={onFormKeys}>
      <div className="console-head">
        <span className="w-tape">{isEdit ? "Edit intention" : "New task"}</span>
        <div style={{ flex: 1 }} />
        <button className="icon-btn" onClick={onClose} aria-label="Close"><Icon name="close" size={13} /></button>
      </div>
      <div className="v-form-fields">
        <input ref={titleRef} className="field" aria-label="Task title" placeholder="Give this intention a name…"
          value={title} onChange={(e) => setTitle(e.target.value)}
          onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey && !e.metaKey && !e.ctrlKey) submit(); }}
          style={{ fontSize: 15, padding: "10px 12px" }} />
        <textarea className="field" aria-label="Description" placeholder="Description (optional)" value={desc} onChange={(e) => setDesc(e.target.value)} style={{ height: 52 }} />
        <textarea className="field" aria-label="Notes" placeholder="Notes — context, blockers, references" value={notes} onChange={(e) => setNotes(e.target.value)} style={{ height: 40 }} />
        ...
        {recurring === "none" && !isSplit && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            <div>
              <Label>Scheduled {scheduleDate ? "" : "(planner decides)"}</Label>
              <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
                <input type="date" value={scheduleDate} onChange={(e) => setScheduleDate(e.target.value)}
                  className="field field--mono" style={{ padding: "5px 7px", width: 140 }} />
                <button type="button" className="bezel bezel--sm" onClick={() => setScheduleDate(isoDate(effectiveToday(s.meta.dayStartHour || 0)))}>Today</button>
                <button type="button" className="bezel bezel--sm" onClick={() => setScheduleDate(isoDate(addDays(effectiveToday(s.meta.dayStartHour || 0), 1)))}>Tomorrow</button>
