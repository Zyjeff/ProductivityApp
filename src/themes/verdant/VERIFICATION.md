...
- Visual review: 1440 × 1000 desktop and 390 × 844 mobile, screenshots of the running sample workspace.
- Theme-switch soak: all **12** themes, three cycles; exactly one mounted stylesheet and byte-identical persisted `werf_*` values, excluding the selected theme preference.
...
| S1–S8 | Codex chapters; level/summary/calibration surfaces; all 16 achievement cards and real progress; screen history; rollover range; theme picker; backup/export/import/preview controls |
...
- All four primary screens at 390px: no horizontal document overflow. The Almanac intentionally scrolls its day strip horizontally on narrow screens.
...
- Representative solid token contrast ratios: body **14.62:1**, muted text **6.26:1**, primary button **12.29:1**. These are token checks, not an exhaustive accessibility certification.
