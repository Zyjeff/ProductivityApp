...
import { Heading, Glyph, Segments } from "../art.jsx";
...
export function PlanView() {
  ...
  const kbRows = useMemo(()=>agendaDays.flatMap(day=>day.items.map(it=>({taskId:it.taskId,chunkDate:day.iso,done:!!(it.done||tasksById.get(it.taskId)?.completed)}))),[agendaDays,tasksById]);
  ...
  return <div className="v-almanac">
    <Heading eyebrow="THE ALMANAC" title="Give your time a shape." subtitle={pending.length+' pending · '+totalH.toFixed(1)+'h estimated · '+unscheduled.length+' unscheduled · '+recurringHours.toFixed(1)+'h daily reserve'}>
