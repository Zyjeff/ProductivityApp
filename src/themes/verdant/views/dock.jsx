...
import { Heading, Glyph, Seal, Segments } from "../art.jsx";
...
export function DockView() {
  ...
  return <div className="v-collections"><Heading eyebrow="YOUR COLLECTIONS" title="Ideas, taking form." subtitle={active.length+' active projects · '+ready.length+' ready to launch · '+launched.length+' launched'}><div className="slab-types" role="group" aria-label="Collection view">{[['projects','Projects'],['library','Task library']].map(([id,label])=><button key={id} className={tab===id?'on':''} aria-pressed={tab===id} onClick={()=>setUI({dockFilter:{...filter,tab:id},cursor:null})}>{label}</button>)}</div></Heading><div className="v-collection-capture"><CaptureSlab/></div>{tab==='projects'?<BerthsTab/>:<LibraryTab/>}<details className="v-archive"><summary><Glyph seed={2} size={18}/> Completed collections <span>{launched.length}</span></summary>{launched.length?<ManifestRail launched={launched}/>:<p>Your first finished project will live here.</p>}</details></div>;
}
            ...
            {open && (
              <div className="fade-in" style={{ marginTop: 8, padding: "10px 10px 10px 12px", background: "var(--well)", boxShadow: "inset 0 1px 0 var(--line-dim)" }}>
                ...
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  ...
                  <button className="bezel bezel--sm" onClick={() => unshipProject(p.id)} title="Back to the workspace">Unlaunch</button>
                </div>
              </div>
            )}
          </div>
                  ...
                  <div className="ledger" style={{ marginTop: 12, background: "var(--well)" }}>
                    {children.map((t) => (
                      <div key={t.id} className="lrow" style={{ minHeight: 36, "--row-lamp": t.completed ? "var(--line-dim)" : p.color }}>
                        ...
                        <button className="icon-btn" onClick={() => detachFromProject(t.id)} title="Detach from project" aria-label="Detach"><Icon name="close" size={11} /></button>
                      </div>
                    ))}
                  </div>
