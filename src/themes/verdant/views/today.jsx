...
import { Seal, Glyph, Heading, Segments } from "../art.jsx";
...
export function TodayView() {
  ...
  return <div className="v-today">
    <Heading eyebrow={D.parseIsoDate(todayIso).toLocaleDateString('en-GB',{weekday:'long',day:'numeric',month:'long'})} title={allDone?'A day well spent.':tasks.length===0?'Make room for good work.':hourNow<12?'A fresh kind of focus.':hourNow<18?'A little more progress.':'An evening of possibility.'} subtitle="Small intentions. Quiet momentum.">
      <span className="v-subtle-count">{pending.length} pending <i/> {lineup.length} lined up {readyToShip.length>0&&<> <i/> {readyToShip.length} ready</>}</span>
      <button className="bezel" onClick={()=>setUI({endOfDayOpen:true})}><Icon name="moon"/> Close day</button>
    </Heading>
    {dayClosed&&<div className="v-notice">This day is sealed. Your work is saved.<button className="link" onClick={reopenDay}>Reopen day ↗</button></div>}
    {dueWeek&&<div className="v-notice"><Glyph seed={5} size={17}/><span>A moment to reflect on last week.</span><button className="link" onClick={()=>openReview(dueWeek)}>Weekly reflection</button><button className="icon-btn" aria-label="Dismiss review prompt" onClick={()=>patchReview(dueWeek,{promptDismissed:true})}><Icon name="close"/></button></div>}
    <div className="v-sanctum-grid">
      <section className="v-ritual-stone">
        <div className="v-stone-heading"><span className="v-eyebrow">THE FOCUS STONE</span><button className="v-motion-toggle" aria-pressed={alive} title="Toggle ambient motion" aria-label="Toggle ambient motion" onClick={()=>setAlive(x=>!x)}>{alive?'✦':'✧'}</button></div>
        <Seal pct={lineup.length?planDone/lineup.length*100:0} active={alive}/>
        <div className="v-stone-copy"><span className="v-eyebrow">{nowRow?'YOUR NEXT SMALL STEP':allDone?'THE SEAL IS COMPLETE':'A SPACE FOR INTENTION'}</span>
          <h2>{nowRow?nowRow.task.title:allDone?'Everything, in its own time.':'One thing at a time.'}</h2>
          <p>{nowRow?<>{nowProject?.title||'Personal task'} · {nowRow.chunk?.hours??D.taskHours(nowRow.task)}h{D.todayFocusMs(nowRow.task,hour)>0?' · '+D.fmtMs(D.todayFocusMs(nowRow.task,hour))+' focused':''}</>:allDone?'All of today’s tasks are complete.':'Capture a task to begin your next ritual.'}</p>
          {nowRow?.chunkNote&&<p>{nowRow.chunkNote}</p>}
          {nowRow?<div className="v-focus-actions"><button className="switch" onClick={()=>setUI({focusTaskId:nowRow.task.id})}><Icon name="focus"/> Begin focus <Kbd>F</Kbd></button><button className="bezel" title="Complete current task" aria-label="Complete current task" onClick={()=>setCompletion(nowRow.task.id,{done:true,chunkDate:nowRow.chunk?todayIso:null})}><Icon name="check"/></button></div>:allDone?<button className="switch" onClick={()=>setUI({endOfDayOpen:true})}>Seal the day <Icon name="check"/></button>:<button className="switch" onClick={()=>setUI({formOpen:true,editingTaskId:null})}>Set an intention <Icon name="plus"/></button>}
        </div>
        <div className="v-stone-foot"><Segments pct={lineup.length?planDone/lineup.length*100:0} count={24}/><div><span>{planDone} / {lineup.length} COMPLETE</span><span>{nowRow?'KEEP THE MOMENTUM':'TAKE YOUR TIME'}</span></div></div>
        {nextRow&&<div className="v-next-task"><span>THEN</span><span>{nextRow.task.title}</span><Icon name="chevronR"/></div>}
      </section>
      <section className="v-workspace"><CaptureSlab scheduleToday/>
                  {missed.length > 0 && (
          ...
          <div className="section-head">
            <div className="w-stencil-row" style={{ flex: 1 }}>
              <span className="w-stencil">Your intentions</span>
            </div>
            <EnergyToggle />
        ...
        <div className="v-brief">          {/* the telegram brief */}
    ...
    <section className="v-daily-metrics" aria-label="Today's activity">
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {readyToShip.slice(0, 4).map((p) => (
                  <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 9 }}>
                    ...
                    <button className="switch switch--launch switch--sm" onClick={() => setUI({ view: "dock" })}>Launch</button>
                  </div>
                ))}
              </div>
            </div>
    ...
    <div className="v-page-foot"><Glyph seed={6} size={16}/><span>Great things grow from small beginnings.</span><Glyph seed={2} size={16}/></div>
  </div>;
}
