...
function Inner() {
  ...
  return (
    <div onClick={() => setUI({ endOfDayOpen: false })}
      ...
      <div onClick={(e) => e.stopPropagation()} className="console v-receipt" style={{ width: "100%", maxWidth: 440 }}>
        <div className="console-head">
          <span className="w-tape"><Icon name="moon" size={10} /> Seal the day</span>
          <div style={{ flex: 1 }} />
          <button className="icon-btn" onClick={() => setUI({ endOfDayOpen: false })} aria-label="Close"><Icon name="close" size={12} /></button>
        </div>
        <div style={{ padding: "16px 20px 20px" }}>
          <div className="w-display" style={{ fontSize: 23, marginBottom: 16 }}>
            {completedToday.length > 0 ? "A day well spent." : "Leave a little room for tomorrow."}
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 18 }}>
            ...
            <div style={{ background: "var(--well)", boxShadow: "inset 0 1px 0 rgba(0,0,0,0.5)", padding: "12px 14px" }}>
              ...
              <div className="w-num" style={{ fontSize: 22, fontWeight: 600, color: "var(--amber-hot)" }}>{xpToday}</div>
            </div>
          </div>

          {completedToday.length > 0 && (
            <div style={{ marginBottom: 18 }}>
              <div className="w-stencil" style={{ marginBottom: 8 }}>In the books</div>
              <div className="scrolly" style={{ display: "flex", flexDirection: "column", gap: 4, maxHeight: 140, overflowY: "auto" }}>
          ...
          {unfinished.length > 0 && (
            <div style={{ marginBottom: 18 }}>
              ...
              <div className="scrolly" style={{ display: "flex", flexDirection: "column", gap: 4, maxHeight: 110, overflowY: "auto" }}>
                {unfinished.map((t) => (
          ...
          <div style={{ marginBottom: 18 }}>
            ...
            {screenEntry.mobileHours != null ? (
              ...
              <div style={{ display: "flex", gap: 6 }}>
                <input type="number" min="0" max="24" step="0.5" value={mobile} className="field field--mono"
                ...
                <button className="bezel bezel--sm" disabled={!mobile}
                  onClick={() => { patchScreenToday({ mobileHours: parseFloat(mobile) || 0 }); setMobile(""); }}>Log</button>
              </div>
            )}
          </div>

          <button className="switch" style={{ width: "100%" }}
          ...
          <div className="w-num" style={{ fontSize: 9.5, color: "var(--fg-faint)", marginTop: 10, textAlign: "center", letterSpacing: "0.08em" }}>
            A DAY WELL SPENT. THAT'S ENOUGH FOR TODAY.
          </div>
        </div>
      </div>
    </div>
