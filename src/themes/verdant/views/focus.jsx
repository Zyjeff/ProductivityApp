...
import { Seal, Glyph } from "../art.jsx";
...
function Tunnel({ taskId }) {
  ...
  return <div className="tunnel v-focus" role="dialog" aria-label="Focus mode" aria-modal="true"><header><span className="v-focus-brand"><Glyph seed={4}/> VERDANT / RITUAL</span><button className="bezel" onClick={()=>setUI({focusTaskId:null})}>Return to workspace <Kbd>Esc</Kbd></button></header><div className="v-focus-center"><span className="v-eyebrow">{running?'ONE TASK. YOUR FULL ATTENTION.':'A MOMENT OF STILLNESS.'}</span><div className="v-focus-clock"><Seal active={running} pct={Math.min(100,elapsed/(D.taskHours(task)*3600000)*100)}><div className="v-clock-content"><span className="tunnel-timer">{timeStr}</span><small>{running?'FOCUSING':'PAUSED'}</small></div></Seal></div><h1>{task.title}</h1><p>{project?.title||'Personal task'} · {task.difficulty} · ~{D.taskHours(task)}h</p><div className="v-focus-transport"><button className="bezel" onClick={()=>setRunning(r=>!r)}>{running?'Pause':'Resume'} <Kbd>Space</Kbd></button><button className="icon-btn" title="Reset timer" aria-label="Reset timer" disabled={elapsed===0} onClick={()=>{const current=baseRef.current+(startRef.current==null?0:Date.now()-startRef.current);logSession(taskId,Math.max(0,current-sessionSeedRef.current));sessionSeedRef.current=0;baseRef.current=0;latestRef.current=0;startRef.current=running?Date.now():null;setElapsed(0);saveFocusTime(taskId,0);}}><Icon name="reset"/></button><button className="switch" onClick={complete}>Mark complete <Icon name="check"/></button></div></div><footer><span>A little less noise. A little more you.</span>{nextTask&&<button onClick={()=>setUI({focusTaskId:nextId})}><span>UP NEXT</span> {nextTask.title} <span>Skip ↗</span></button>}</footer></div>;
}
