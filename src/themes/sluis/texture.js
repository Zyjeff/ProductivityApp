// texture.js — Sluis's seeded dither engine, copied (per theme rules)
// and retuned for the concrete-and-water palette: pale grounds, deep
// teal water, soft ink. Rendered ONCE per parameter set, cached as a
// data URL, never touched per frame.
...
export function cssColor(c, fallback = "#0b7d9e") {
...
export function tex(opts) {
  ...
  const { seed = "werf", algo = "bayer", w = 96, h = 96, cell = 2, fg = "#0b7d9e", bg = null, alpha = 1 } = opts;
...
// Water grain: a soft teal fade used under chamber fills and pound
// water — dense at the waterline, dissolving downward.
export function waterGrain(seedString, depth = 26, from = 0.30) {
  return edgeTex({ seed: "water-" + seedString, side: "top", depth, fg: "#095b70", from, cell: 2 });
}

// Statement plate: ink dither creeping from the edges of an amber
// surface; the center stays clean for text (the legibility law).
export function statementBg(seedString, ink = "#10222b", amber = "#f0a13e") {
...
export function ghostFill(seedString, w = 260) {
  return tex({ seed: "ghost-" + seedString, algo: "bayer", w, h: 168, cell: 3, fg: "#c9d8d5", grad: { from: 0.5, to: 0.06, dir: "down" } });
}
