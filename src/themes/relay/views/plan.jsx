// plan.jsx — Lattice, Sluis cut: a stepped profile of cells, one
// per day, heat = load ÷ capacity, lock gates in the gaps. ONE
// detail tray (Lattice) for the selected day holds every chunk action —
// complete, edit, hours/note, unschedule, drag out. The Lattice
...
import { Icon, Kbd, Lamp, Meter, Stamp, ResolveBtn, Eyebrow, EmptyState, MOD } from "../components.jsx";
...
export function PlanView() {
  ...
  const capacityInput = (day, val, style) => (
    <input type="number" min="0" max="10" step="0.5" defaultValue={val} autoFocus
      ...
      className="r-field" style={{ padding: "3px 6px", fontSize: 12, ...style }} />
  );
  ...
  const chunkCard = (day, it) => {
    ...
    return (
      <div key={it.taskId} className="r-chunk"
        ...
        <span
          ...
          title={chunkDone ? "Locked through" : "Drag onto a cell to move"}
        ...
        <ResolveBtn size={20} done={chunkDone}
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
            ...
            {isSplit && <Stamp tone="var(--amber-deep)" style={{ flexShrink: 0 }}>{idx + 1}/{chunks.length}</Stamp>}
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 7, marginTop: 3 }}>
            {project && <span className="r-num" style={{ fontSize: 9, color: project.color, flexShrink: 0 }}>{project.title.toUpperCase()}</span>}
        ...
        <input type="number" min="0.25" max="24" step="0.25" placeholder="hrs" disabled={chunkDone}
          ...
          className="r-field" style={{ width: 56, padding: "3px 5px", fontSize: 11, textAlign: "center", flexShrink: 0 }} />
        <input type="date" value={day.iso} disabled={chunkDone}
          ...
          className="r-field" style={{ width: 126, padding: "3px 5px", fontSize: 11, flexShrink: 0 }} />
        <button className="r-icon-btn" onClick={() => dismissChunk(t.id, day.iso)} title="Unschedule (undoable)" aria-label="Unschedule" style={{ flexShrink: 0 }}>
    ...
    <div>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 14, flexWrap: "wrap" }}>
          <h1 className="r-display" style={{ fontSize: 30, margin: 0 }}>Lattice</h1>
          <div className="r-num" style={{ fontSize: 10, color: "var(--fg-faint)", letterSpacing: ".06em", paddingBottom: 4 }}>
      ...
      <div style={{ marginBottom: 22 }}>
        <Eyebrow>Capacity — hours per day</Eyebrow>
          ...
          {D.WEEK_ALL.map((day) => {
            ...
            return (
              <div key={day} style={{
                ...
                <div className="r-lab" style={{ color: isTodayDow ? "var(--amber-hot)" : undefined, marginBottom: 6 }}>
                  {day.slice(0, 3)}
                </div>
                {editingCap === day ? (
                  capacityInput(day, val, { width: "100%", marginBottom: 6 })
                ) : (
                  <button onClick={() => setEditingCap(day)} className="r-btn r-btn--sm r-btn--ghost"
        ...
        {recurring.length > 0 && (
          <div className="r-num" style={{ margin: "8px 0 0", fontSize: 9.5, color: "var(--fg-faint)" }}>
      ...
      {/* ── Lattice ── */}
      <Eyebrow right={<span className="r-lab" style={{ fontSize: 8.5 }}>click selects · drag chunks onto a cell</span>}>
        Lattice
      </Eyebrow>

      {allEmpty ? (
        <EmptyState caption="no lattice traffic">
          <div className="r-display" style={{ fontSize: 17, color: "var(--fg)", marginBottom: 8 }}>Lattice is empty.</div>
          <div>Route packets in the Feed, then chart them here — by hand (drag) or via AI schedule.</div>
        </EmptyState>
      ) : (
        <>
          <div className="r-day-lattice">
            {agendaDays.map((day) => {
              ...
              return (
                <div key={day.iso} style={{ position: "relative", flexShrink: 0, display: "flex" }}>
                  <div
                    className={"r-day-cell"
                      + (day.isToday ? " r-day-cell--today" : "")
                      + (day.locked ? " r-day-cell--locked" : "")
                      + (isSel ? " r-day-cell--sel" : "")
                      + (over ? " r-day-cell--over" : "")
                      + (day.cap === 0 ? " r-day-cell--off" : "")}
                    role="button" tabIndex={0} aria-pressed={!!isSel}
                    title={`${D.fmtIsoShort(day.iso)} — ${day.locked ? "locked; " : ""}click to open in Lattice`}
                    ...
                    style={{
                      "--heat": loadPct + "%",
                      ...(dragOver === day.iso ? { outline: "2px dashed var(--lime)" } : null),
                    }}>
                    <div className="r-lab">{day.wd.slice(0, 3)}</div>
                    <div className="r-num">{day.d.getDate()}</div>
                    <div className="r-lab" style={{ fontSize: 8 }}>
                      {day.locked ? "LOCK" : day.cap === 0 ? "OFF" : `${dayH.toFixed(1)}/${day.cap}`}
                    </div>
                  </div>
                </div>
              );
          ...
          <div style={{ display: "flex", justifyContent: "center", marginTop: 10 }}>
            <button className="r-btn r-btn--sm r-btn--ghost" onClick={() => setHorizon((h) => h + 28)}>+4 weeks</button>
          </div>

          {/* ── Lattice: the selected day ── */}
          {selDay && (() => {
            const { dayH, loadPct, over } = dayLoad(selDay);
            const loadTone = over ? "var(--port)" : dayH === selDay.cap && selDay.cap > 0 ? "var(--starboard)" : "var(--channel)";
            return (
              <div className="r-day-tray">
                <div style={{ display: "flex", alignItems: "flex-start", gap: 12, flexWrap: "wrap", marginBottom: 12 }}>
                  <div style={{ flex: 1, minWidth: 190 }}>
                    <div className="r-lab" style={{ marginBottom: 4 }}>Lattice</div>
                    <div className="r-display" style={{ fontSize: 20, display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                      {D.fmtIsoShort(selDay.iso)}
                      {selDay.isToday && <Stamp tone="var(--amber-deep)">VANDAAG</Stamp>}
                      {selDay.locked && <Stamp tone="var(--port)">LOCKED</Stamp>}
                    </div>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    <span className="r-lab">Schaal</span>
                    {editingCap === selDay.wd ? (
                      capacityInput(selDay.wd, selDay.cap, { width: 64 })
                    ) : (
                      <button className="r-btn r-btn--sm r-btn--ghost" onClick={() => setEditingCap(selDay.wd)}
                        title={`Edit ${selDay.wd} capacity (0–10h)`}>
                        {selDay.cap === 0 ? "off" : `${selDay.cap}h`}
                      </button>
                    )}
                  </div>
                  <button className={"r-btn r-btn--sm " + (selDay.locked ? "r-btn--amber" : "r-btn--ghost")}
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
                  <span className="r-num" style={{ fontSize: 10.5, whiteSpace: "nowrap", color: over ? "var(--port)" : "var(--fg-dim)" }}>
                  ...
                  {selDay.cap > 0 && (
                    <span className="r-num" style={{ fontSize: 10.5, color: "var(--fg-faint)", flexShrink: 0 }}>
                ...
                {selDay.items.length === 0 ? (
                  <div style={{ padding: "12px 4px 6px", textAlign: "center", color: "var(--fg-faint)", fontSize: 12.5 }}>
                    {selDay.cap === 0 ? "OFF — no capacity" : "clear day"}
                  </div>
                ) : (
      ...
      {/* ── Lifecycle stage rail: Discovery / Validation / Fixing ── */}
      <div className="r-lattice-shell" style={{ marginTop: 18 }}>
        <div className="r-stage-rail" aria-label="AI ops stages">
          <button type="button" className={"r-stage-btn" + (busy === "schedule" || instructionOpen ? " r-stage-btn--on" : "") + (busy === "schedule" ? " r-stage-btn--busy" : "")}
            disabled={!!busy || unscheduled.length === 0}
            onClick={() => { setInstructionOpen(false); runSchedule(""); }}>
            Discovery <span className="r-lab">{unscheduled.length || "—"}</span>
          </button>
          <button type="button" className={"r-stage-btn" + (busy === "optimize" ? " r-stage-btn--on r-stage-btn--busy" : "")}
            disabled={!!busy || !(plan || []).length}
            onClick={runOptimize}>
            Validation
          </button>
          <button type="button" className={"r-stage-btn" + (busy === "replan" ? " r-stage-btn--on r-stage-btn--busy" : "")}
            disabled={!!busy || !(plan || []).length}
            onClick={() => document.getElementById("r-replan")?.querySelector("input")?.focus()}>
            Fixing
          </button>
          <div style={{ marginTop: 8, padding: "8px 10px" }}>
            <span className="r-num" style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 9.5, color: "var(--fg-faint)" }}>
              <Lamp status={aiStatus} detail={aiDetail} /> {aiStatus === "off" ? "AI OFF" : aiStatus === "busy" ? "BUSY" : "LINK"}
            </span>
          </div>
        </div>
        <div>
          <div className="r-plate" style={{ marginBottom: 12 }}>
            <div className="r-lab r-lab--lit" style={{ marginBottom: 10 }}>Schedule ops</div>
            <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
              {unscheduled.length > 0 && (
                <>
                  <button className="r-btn r-btn--sm" disabled={!!busy} onClick={() => runSchedule("")}>
                    {busy === "schedule" ? "Scheduling…" : `Schedule ${unscheduled.length} pending`}
                  </button>
                  <button className="r-btn r-btn--sm r-btn--ghost" disabled={!!busy} onClick={() => setInstructionOpen((v) => !v)}
              ...
              {(plan || []).length > 0 && (
                <button className="r-btn r-btn--sm r-btn--ghost" disabled={!!busy} onClick={runOptimize}
              ...
              {unscheduled.length === 0 && (plan || []).length === 0 && (
                <span style={{ fontSize: 12, color: "var(--fg-faint)" }}>
                  Nothing to schedule — route packets in the Feed first.
                </span>
              )}
            </div>
            {instructionOpen && (
              <div style={{ marginTop: 12 }}>
                <textarea autoFocus value={instruction} disabled={!!busy} className="r-field"
                  ...
                  placeholder="e.g. spread over the next 2 weeks · finish by Friday"
                  style={{ height: 56, marginBottom: 10 }} />
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  <button className="r-btn r-btn--sm" disabled={!!busy} onClick={() => runSchedule(instruction)}>
                    {busy === "schedule" ? "Scheduling…" : "Schedule"}
                  </button>
                  <button className="r-btn r-btn--sm r-btn--ghost" disabled={!!busy} onClick={() => { setInstructionOpen(false); setInstruction(""); }}>Cancel</button>
                  <span className="r-num" style={{ fontSize: 9.5, color: "var(--fg-faint)", marginLeft: "auto" }}>
                    <Kbd>{MOD}</Kbd><Kbd>↵</Kbd>
                  </span>
                </div>
              </div>
          ...
          {(plan || []).length > 0 && (
            <div className="r-plate" id="r-replan">
              <div className="r-lab r-lab--lit" style={{ marginBottom: 10 }}>Replan console</div>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <input value={replanText} disabled={!!busy} className="r-field" style={{ flex: 1, minWidth: 0 }}
                  ...
                  placeholder={busy === "replan" ? "Re-charting…" : "e.g. move hard work to Wednesday…"}
                  aria-label="Replan instruction" autoComplete="off" />
                <button className="r-btn r-btn--sm" disabled={!replanText.trim() || !!busy} onClick={runReplan} style={{ flexShrink: 0 }}>
                  {busy === "replan" ? "…" : "Transmit"}
                </button>
              </div>
              {!replanText && !busy && (
                <div style={{ display: "flex", gap: 5, flexWrap: "wrap", marginTop: 8, alignItems: "center" }}>
                  <span className="r-lab" style={{ marginRight: 2 }}>Try</span>
                  {["Spread the client work over 2 weeks", "Push everything past Friday", "Move design tasks earlier"].map((ex) => (
                    <button key={ex} onClick={() => setReplanText(ex)} className="r-stamp r-stamp--pick"
      ...
      {missed.length > 0 && (
        <div className="r-plate" style={{ marginTop: 14, display: "flex", alignItems: "center", gap: 10, borderColor: "var(--amber)" }}>
          <Stamp tone="var(--amber-deep)">ORPHANS</Stamp>
          <span style={{ fontSize: 12, color: "var(--fg-dim)" }}>
            {missed.length} item{missed.length > 1 ? "s" : ""} from past days — triage in
          </span>
          <button className="r-link" onClick={() => setUI({ view: "today" })}>Feed</button>
        </div>
      )}
