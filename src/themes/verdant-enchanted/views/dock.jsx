...
import { Heading, Glyph, Seal, Segments, BookArt, PixelText, Sprite, RuneRule } from "../art.jsx";
...
export function DockView() {
  ...
  return <div className="v-collections"><Heading eyebrow="THE LIBRARY" title={<PixelText size={32}>YOUR GRIMOIRES</PixelText>} subtitle={active.length+' active projects · '+ready.length+' ready to seal · '+launched.length+' sealed'}><div className="slab-types" role="group" aria-label="Collection view">{[['projects','Projects'],['library','Task library']].map(([id,label])=><button key={id} className={tab===id?'on':''} aria-pressed={tab===id} onClick={()=>setUI({dockFilter:{...filter,tab:id},cursor:null})}>{label}</button>)}</div></Heading><div className="v-collection-capture"><CaptureSlab/></div>{tab==='projects'?<BerthsTab/>:<LibraryTab/>}<details className="v-archive"><summary><Glyph seed={2} size={18}/> Sealed grimoires <span>{launched.length}</span></summary>{launched.length?<ManifestRail launched={launched}/>:<p>Your first finished project will live here.</p>}</details></div>;
}
            ...
            {open && (
              <div className="fade-in" style={{ marginTop: 8, padding: "10px 10px 10px 12px", background: "var(--well)", boxShadow: "inset 0 1px 0 var(--line-dim)" }}>
                ...
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  ...
                  <button className="bezel bezel--sm" onClick={() => unshipProject(p.id)} title="Back to the workspace">Reopen grimoire</button>
                </div>
              </div>
            )}
          </div>
...
function deterministicLaunchDraft(stats) {
  if (!stats) return "";
  return `Completed with ${stats.doneTasks}/${stats.tasks} tasks done and ${stats.xp} XP earned. ` +
    `Estimated ${stats.estHours}h${stats.actualMs ? `, actually focused ${D.fmtMs(stats.actualMs)}` : ""}. ` +
    `From first intention to completion: ${stats.spanDays} day${stats.spanDays === 1 ? "" : "s"}.`;
}

export function LaunchNoteDialog() {
...
function LaunchNoteInner({ projectId }) {
  ...
  useEffect(() => {
    ...
    aiLaunchNote(project, stats || {}).then((text) => {
      if (alive && text && !editedRef.current) setNote(text);
    ...
    <div className="backdrop" onClick={() => setUI({ launchNoteFor: null })}
      ...
      <div onClick={(e) => e.stopPropagation()} className="console fade-in" style={{ width: "100%", maxWidth: 470 }}>
        <div className="console-head">
          <span className="w-tape w-tape--starboard"><Icon name="ship" size={10} /> Completion journal entry</span>
          <div style={{ flex: 1 }} />
        ...
        <div style={{ padding: "16px 18px 18px" }}>
          ...
          <textarea className="field" value={note} onChange={(e) => { editedRef.current=true; setNote(e.target.value); }}
            placeholder="What did you create, and what did it take?"
...
function BerthsTab() {
  ...
  useEffect(() => {
    ...
    const frame = requestAnimationFrame(() => workbenchRef.current?.scrollIntoView({
  ...
  const projectRows=useMemo(()=>{const p=projects.find(p=>p.id===expandedId);return (p?.childTaskIds||[]).map(id=>({taskId:id,chunkDate:null,done:!!tasksById.get(id)?.completed}));},[projects,tasksById,expandedId]);
  useEffect(()=>{registerActiveList('projects',projectRows);return()=>registerActiveList('projects',[]);},[projectRows]);
  ...
  return (
    <>
      ...
      {active.length === 0 && !adding && (
        <EmptyState caption="room for something new">
          {activeAnyType && typeFilter !== "All"
            ? <>No {D.PROJECT_TYPES.find((t) => t.id === typeFilter)?.label.toLowerCase()} projects in your workspace.</>
            : <>No collections yet. Create a project, or capture with the Project mode.</>}
        </EmptyState>
      )}

      <div className="v-project-cards">{active.map(p=>{const pct=D.progressOfProject(p,tasksById),count=(p.childTaskIds||[]).length;return <button key={p.id} className={'v-project-card'+(expandedId===p.id?' is-selected':'')} onClick={()=>setExpandedId(expandedId===p.id?null:p.id)} aria-expanded={expandedId===p.id}><span className="v-card-type">{D.PROJECT_TYPES.find(t=>t.id===p.type)?.label||'Other'}<span>{Math.round(pct)}%</span></span><BookArt seed={p.id} pct={pct} size={155}/><h2>{p.title}</h2><p>{count} tasks {pct===100&&count>0?'· Ready to launch':''}</p><Segments pct={pct}/><span className="v-card-open">{expandedId===p.id?'Close workbench':'Open grimoire'} <Icon name="chevronR"/></span></button>;})}<button className="v-project-card e-blank-grimoire" aria-label="Create a grimoire" onClick={()=>setAdding(true)}><BookArt size={155} seed={7}/><span className="v-card-type">THE NEXT CHAPTER</span><h2>An unwritten grimoire</h2><p>Make room for a new project.</p><span className="v-card-open"><Icon name="plus" size={11}/> Create grimoire</span></button></div>
      <div className="v-project-workbench" ref={workbenchRef}>
        {active.filter(p=>p.id===expandedId).map((p) => {
            ...
            <div key={p.id} className="berth fade-in" style={{ "--u-fade": plateFade("berth-" + p.id, "#1c2434", 0.35) }}>
              ...
              <div className="berth-head" onClick={() => setExpandedId(isOpen ? null : p.id)}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div className="berth-meta">
                    ...
                    {pct === 100 && children.length > 0 && <Stamp tone="var(--starboard)">COLLECTION COMPLETE</Stamp>}
                  </div>
                </div>
                {isReady && (
                  <button className="switch switch--launch" onClick={(e) => { e.stopPropagation(); shipProject(p.id); }}>
                    <Icon name="ship" size={12} /> Seal grimoire
                  </button>
                )}
              ...
              {isOpen && (
                <div className="fade-in" style={{ padding: "0 18px 16px" }}>
                  ...
                  <div className="e-grimoire-tasks">{children.map((t,i)=><article key={t.id} className={'e-grimoire-leaf'+(t.completed?' is-complete':'')}><div className="e-leaf-number"><Glyph seed={i} size={16}/><span>INTENTION {String(i+1).padStart(2,'0')}</span><HLight done={t.completed} onComplete={()=>setCompletion(t.id,{done:true})} onReopen={()=>setCompletion(t.id,{done:false})}/></div><button className="e-leaf-title" onClick={()=>setUI({editingTaskId:t.id,formOpen:true})}>{t.title}</button><div className="e-leaf-foot"><span>{D.taskHours(t)}h · {t.xp} XP</span><button className="icon-btn" onClick={()=>detachFromProject(t.id)} aria-label="Detach" title="Detach from project"><Icon name="close" size={10}/></button></div></article>)}</div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 14, paddingTop: 12, boxShadow: "inset 0 1px 0 var(--line-dim)", flexWrap: "wrap" }}>
                    ...
                    <button className="bezel bezel--sm bezel--port" onClick={() => deleteProject(p.id)}>Delete project</button>
                    {isReady && (
...
function LibraryTab() {
  ...
  return (
    <>
            ...
            {parsed.map((p, i) => {
              ...
              return (
                <div key={i} onClick={() => setImportSel((prev) => { const n = new Set(prev); n.has(i) ? n.delete(i) : n.add(i); return n; })}
                  ...
                  <HLight square done={on} onComplete={() => setImportSel(prev => new Set([...prev, i]))} onReopen={() => setImportSel(prev => new Set([...prev].filter(n => n !== i)))} label={"Include " + p.title} />
                  <div style={{ flex: 1, minWidth: 0 }}>
      ...
      {list.length === 0 ? (
        <EmptyState caption="nothing in your workspace">Nothing matches.</EmptyState>
      ) : (
                        ...
                        <div style={{ marginBottom: isSplit ? 8 : 0 }}>
                          {berthingId === t.id ? (
                            <div style={{ display: "flex", gap: 5, flexWrap: "wrap", alignItems: "center" }}>
                              <span className="w-stencil" style={{ fontSize: 8.5 }}>Assign to</span>
                              {activeProjects.map((p) => (
                                <StampPick key={p.id} color={p.color} onClick={() => berthTask(t.id, p.id)}>{p.title}</StampPick>
                              ))}
                              <button className="icon-btn" onClick={() => setBerthingId(null)} aria-label="Cancel project assignment"><Icon name="close" size={10} /></button>
                            </div>
                          ) : (
                            <button className="bezel bezel--sm" onClick={() => setBerthingId(t.id)} title="Assign to a project">
                              <Icon name="anchor" size={10} /> Add to grimoire
                            </button>
                          )}
                        </div>
                      )}
