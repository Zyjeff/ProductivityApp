// taskform.jsx — the Revision sheet. All behavior identical to the
// core contract: deterministic score follows difficulty until XP is
// touched, uncommitted step/tag text commits on submit, "Loft it"
// promotes to a project.
...
function TaskForm({ initial, isEdit, onClose }) {
  ...
  return (
    <div className="lf-console lf-fade" onKeyDown={onFormKeys}>
      <div className="lf-console-head">
        <span className="w-tape">{isEdit ? "Revision" : "New line"}</span>
        <div style={{ flex: 1 }} />
        <button className="lf-icon-btn" onClick={onClose} aria-label="Close"><Icon name="close" size={13} /></button>
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 12, padding: 18 }}>
        <input ref={titleRef} className="lf-field" placeholder="What needs drawing?"
          value={title} onChange={(e) => setTitle(e.target.value)}
          onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey && !e.metaKey && !e.ctrlKey) submit(); }}
          style={{ fontSize: 15, padding: "10px 12px" }} />
        <textarea className="lf-field" placeholder="Description (optional)" value={desc} onChange={(e) => setDesc(e.target.value)} style={{ height: 52 }} />
        <textarea className="lf-field" placeholder="Notes — context, blockers, references" value={notes} onChange={(e) => setNotes(e.target.value)} style={{ height: 40 }} />
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center", opacity: isSplit ? 0.5 : 1 }}>
              <label style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 12, color: "var(--fg-dim)" }}>
                ...
                <input type="number" min="0.25" max="24" step="0.25" value={hours} disabled={isSplit}
                  ...
                  className="lf-field lf-field--mono" style={{ width: 68, textAlign: "center" }} />
              </label>
              <label style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 12, color: "var(--fg-dim)" }}>
        ...
        {recurring === "none" && !isSplit && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            <div>
              <Label>On the board {scheduleDate ? "" : "(planner decides)"}</Label>
              <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
                <input type="date" value={scheduleDate} onChange={(e) => setScheduleDate(e.target.value)}
                  className="lf-field lf-field--mono" style={{ padding: "5px 7px", width: 140 }} />
                <button type="button" className="lf-bezel lf-bezel--sm" onClick={() => setScheduleDate(isoDate(new Date()))}>Today</button>
                <button type="button" className="lf-bezel lf-bezel--sm" onClick={() => setScheduleDate(isoDate(addDays(new Date(), 1)))}>Tomorrow</button>
                {scheduleDate && <button type="button" className="lf-bezel lf-bezel--sm" onClick={() => setScheduleDate("")} style={{ color: "var(--fg-faint)" }}>Clear</button>}
              </div>
            </div>
            <div>
              ...
              <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                <input type="date" value={deadline} onChange={(e) => setDeadline(e.target.value)}
                  className="lf-field lf-field--mono" style={{ padding: "5px 7px", width: 140 }} />
                {deadline && <button type="button" className="lf-bezel lf-bezel--sm" onClick={() => setDeadline("")} style={{ color: "var(--fg-faint)" }}>Clear</button>}
              </div>
            </div>
          </div>
        )}
        ...
        {projects.length > 0 && (
          <div>
            <Label>Sheet (project)</Label>
            <select value={projectId} onChange={(e) => setProjectId(e.target.value)}
              className="lf-field" style={{ fontSize: 13, maxWidth: 280 }}>
              <option value="">No project</option>
              {projects.map((p) => (
                <option key={p.id} value={p.id}>{p.title}{p.completedAt ? " (as-built)" : ""}</option>
              ))}
            </select>
          </div>
        )}
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: 5, flexWrap: "wrap", alignItems: "center" }}>
            ...
            <input className="lf-field lf-field--mono" placeholder="+ new tag" style={{ width: 110, fontSize: 11, padding: "4px 8px" }}
        ...
        <div>
          <Label>Steps</Label>
          <input className="lf-field" placeholder="Type a step + Enter" style={{ fontSize: 12 }}
            ...
            <div style={{ marginTop: 8, display: "flex", flexDirection: "column", gap: 4 }}>
              {subs.map((x) => (
                <div key={x.k} style={{ display: "flex", alignItems: "center", gap: 8, padding: "3px 4px" }}>
                  ...
                  <button className="lf-icon-btn" onClick={() => setSubs((p) => p.filter((y) => y.k !== x.k))} aria-label="Remove step"><Icon name="close" size={11} /></button>
                </div>
              ))}
            </div>
          )}
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 4, alignItems: "center" }}>
          <button className="lf-switch" onClick={submit} disabled={!canSubmit} style={{ flex: 1 }}>
            {isEdit ? "Save revision" : "Pencil in"}
          </button>
          {isEdit && !initial?.projectId && (
            <button
              ...
              title={subs.length === 0 && !stIn.trim()
                ? "Add at least one step first — they become the sheet's tasks"
                : "Take it full-size to the loft floor; each step becomes its own scored task"}
              onClick={() => {
                const finalSubs = commitSubInput(subs);
            ...
            >
              <Icon name="pen" size={12} /> Loft it
            </button>
          )}
          <button className="lf-bezel" onClick={onClose}>Cancel</button>
        </div>
