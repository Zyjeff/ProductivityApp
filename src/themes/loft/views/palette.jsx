// palette.jsx — "Call out a dimension…" — the Loft command line.
// Commands, task search, `>` quick add with the capture grammar,
// cursor-row commands. Identical capabilities to the core contract.
...
function PaletteInner() {
  ...
  const items = useMemo(() => {
    ...
    if (q.startsWith(">")) {
      ...
      return [{
        type: "quickadd",
        id: "quickadd",
        label: raw ? `Pencil in: "${parsed.title || raw}"${chipStr}` : "Quick add — type a task, with !prio #tag @project 2h fri…",
        sub: parsed?.scheduleDate ? "on the sheet " + parsed.scheduleDate : "to the file",
        keepOpen: !raw,
        run: () => {
          if (!raw) return;
  ...
  return (
    <div className="lf-palette-backdrop" onClick={() => setUI({ paletteOpen: false })}>
      <div className="lf-palette lf-fade" onClick={(e) => e.stopPropagation()}>
        <div className="lf-palette-prompt">
          <span className="lf-caret" aria-hidden />
          <input ref={inputRef} value={q} placeholder="Call out a dimension…" autoComplete="off"
          ...
          <span className="w-stencil" style={{ fontSize: 8.5 }}>Call-out</span>
        </div>
        <div className="lf-palette-list lf-scroll">
          {items.length === 0 && <div className="w-num" style={{ padding: "12px 16px", fontSize: 11, color: "var(--fg-faint)" }}>NOTHING IN THE FILE MATCHES.</div>}
          {items.map((item, i) => (
            <button key={item.id} className={"lf-palette-item" + (i === idx ? " is-active" : "")}
