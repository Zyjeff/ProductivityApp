// focus.jsx — the light table: one task under the lamp, the rest of
// the office dark… well, bright. Timer chaining and the session
// ledger behave identically to the core contract.
...
function Tunnel({ taskId }) {
  ...
  return (
    <div className="lf-tunnel" role="dialog" aria-label="Focus mode">
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "18px 28px" }}>
        <button className="lf-bezel" onClick={() => setUI({ focusTaskId: null })}>
          <Icon name="close" size={14} /> Leave the table <Kbd>Esc</Kbd>
        </button>
        <span className="w-stencil">{running ? "Lamp on" : "Lamp dimmed"}</span>
        {nextTask ? (
          <button className="lf-bezel lf-bezel--sm" onClick={() => setUI({ focusTaskId: nextId })}>
            Next line <Icon name="chevronR" size={12} />
          </button>
        ) : <span />}
      </div>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 24, padding: 24 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14, color: "var(--fg-dim)", fontSize: 13 }}>
          <span style={{ width: 12, height: 12, borderRadius: "50%", background: "var(--amber)", boxShadow: "0 0 0 3px var(--amber-ground)" }} />
        ...
        <h1 className="w-display lf-tunnel-title">{task.title}</h1>
        <div className="lf-tunnel-timer">{timeStr}</div>
        <div style={{ display: "flex", gap: 10, marginTop: 4, alignItems: "center" }}>
          <button className="lf-bezel" onClick={() => setRunning((r) => !r)} style={{ minWidth: 130 }}>
            {running ? "Pause" : "Resume"} <Kbd>Space</Kbd>
          </button>
          <button className="lf-bezel" disabled={elapsed === 0}
            onClick={() => { baseRef.current = 0; startRef.current = Date.now(); setElapsed(0); saveFocusTime(taskId, 0); }}>
            <Icon name="reset" size={13} /> Reset
          </button>
          <button className="lf-switch lf-switch--launch" onClick={complete}>
            <Icon name="check" size={13} /> Ink it
          </button>
        </div>
        {nextTask && (
          <div className="lf-fade" style={{ background: "var(--plate)", border: "1px solid var(--line)", padding: "12px 18px", minWidth: 280, textAlign: "left" }}>
            <div className="w-stencil" style={{ marginBottom: 4 }}>Next under the lamp</div>
            <div style={{ fontSize: 13, color: "var(--fg)" }}>{nextTask.title}</div>
          </div>
        )}
      </div>
      <div className="w-stencil" style={{ textAlign: "center", paddingBottom: 22, opacity: 0.7 }}>
        one line at a time
      </div>
    </div>
  );
